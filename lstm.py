...
class LSTMLayer(object):
    def __init__(self, rng, layer_id, shape, X, mask, is_train = 1, p = 0.5, num_sents = 1):
        ...
        #outputs_info here contains initial h and c (pre_h and pre_c in function arguments)
        [h, c], updates = theano.scan(_active_mask,
                                      sequences = [self.X, self.M],
                                      outputs_info = [T.alloc(floatX(0.), 1, num_sents * self.out_size),
                                                      T.alloc(floatX(0.), 1, num_sents * self.out_size)])

        h = T.reshape(h, (self.X.shape[0], num_sents * self.out_size))
        # dropout
        if p > 0:
...
class BdLSTM(object):
    ...
    def __init__(self, rng, layer_id, shape, X, mask, is_train = 1, num_sents = 1, p = 0.5):
        fwd = LSTMLayer(rng, "_fwd_" + layer_id, shape, X, mask, is_train, num_sents, p)
        bwd = LSTMLayer(rng, "_bwd_" + layer_id, shape, X[::-1], mask[::-1], is_train, num_sents, p)
